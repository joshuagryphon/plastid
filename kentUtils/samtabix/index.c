...
ti_index_t *ti_index_load(const char *fn)
{
    ...
    char *fname = NULL;
    if (! strstr(fn + strlen(fn) - 5, ".tbi")) {
		fname = (char*)calloc(strlen(fn) + 5, 1);
		strcat(strcpy(fname, fn), ".tbi");;
	} else {
		fname = strdup(fn);
	}
    BGZF *fp = bgzf_open(fname, "r");
    if (fp == NULL) {
            fprintf(stderr, "[ti_index_load] failed to open \"%s\"\n", fname);
            return NULL;
    }
    idx = ti_index_load_core(fp);
    bgzf_close(fp);
#else
