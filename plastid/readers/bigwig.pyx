...
quantitative values with genomic positions. Because `BigWig`_ files are
indexed, they allow both sequential and random access to data, and require
substantially less memory than unindexed `wiggle`_ or `bedGraph`_ files. Like
`wiggle`_ and `bedGraph`_ formats, `BigWig`_ is an unstranded format, so data
for plus and minus strands must be stored in separate files.
...
#         (e.g. zero, nan, et c. Default: `numpy.nan`) """
    ...
    def __getitem__(self, GenomicSegment roi):
        """Retrieve array of counts at each position in `roi`, in `roi`'s 5' to
        3' direction.
        ...
        -------
        :class:`numpy.ndarray`
            vector of numbers, each position corresponding to a position in
            `roi`, from 5' to 3' relative to `roi`
    ...
    cdef bigWigValsOnChrom* c_get_chromosome_counts(self, str chrom):
        """Retrieve values across an entire chromosome.
         ...
         .. note::
            ...
            The bigWigValsOnChrom* returned by this function MUST be freed by
            you, using bigWigValsOnChromFree() to avoid memoryleaks
        ...
        -------
        *bigWigValsOnChrom
            Chromosome data. chromSize and bufSize fields set to zero if the
            `chrom` is not in the `BigWig`_ file.
        """
        cdef:
            bigWigValsOnChrom* vals
            int i = 0
        ...
        if success == False:
            warnings.warn(
                "Could not retrieve data for chrom '%s' from file '%s'." % (chrom, self.filename),
                DataWarning
            )
            vals.chromSize = 0
            vals.bufSize   = 0
...
def BigWigIterator(BigWigReader reader, maxmem=0):
    ...
    Iterate over records in the `BigWig`_ file, sorted lexically by chromosome
    and position.
